------------------------------------------------------------
-- washburn-postgres.sql
--
-- Create a database and src table in PostgreSQL 
------------------------------------------------------------

-- Create a new bomb user
DROP USER IF EXISTS bomb;
CREATE USER bomb WITH PASSWORD 'giantbomb';


-- Create a new database if its not there...
DROP DATABASE IF EXISTS washdb;
CREATE DATABASE washdb WITH OWNER = bomb;
\connect washdb;
